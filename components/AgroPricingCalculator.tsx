...
import { ricePrices, spicePrices, indianPorts, agriLocations, calculateDistance, calculateLogisticsCost, AgriPrice } from '@/data/commodityPrices';

interface AgroPricingCalculatorProps {
  productType: 'rice' | 'spices';
}

export default function AgroPricingCalculator({ productType }: AgroPricingCalculatorProps) {
  const products = productType === 'rice' ? ricePrices : spicePrices;
  const locations = productType === 'rice' ? agriLocations.rice : agriLocations.spices;

  const [selectedProduct, setSelectedProduct] = useState(products[0].name);
  const [quantity, setQuantity] = useState<number>(100);
  const [selectedSource, setSelectedSource] = useState(locations[0].name);
  ...
  const calculatePrice = () => {
    ...
    const source = locations.find(l => l.name === selectedSource);
    const port = indianPorts.find(p => p.name === selectedPort);

    if (!source || !port) return;

    // Calculate distance
    const distance = calculateDistance(source.lat, source.lng, port.lat, port.lng);

    // Calculate logistics
    const logistics = calculateLogisticsCost(distance, quantity, transportMode);

    // Calculate total FOB price
    // For spices, prices are in USD/KG, convert to per MT
    const pricePerMT = product.unit.includes('KG') ? product.price * 1000 : product.price;
    const basePriceTotal = pricePerMT * quantity;
    const fobPrice = basePriceTotal + logistics.totalLogisticsCost;

    setCalculations({
      productName: product.name,
      variety: product.variety || '',
      basePrice: pricePerMT,
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
      ...
      <p className="text-primary-medium mb-6">
        Calculate FOB prices with logistics costs from {productType === 'rice' ? 'mill/warehouse' : 'processing unit'} to port. Prices updated daily based on market rates.
      </p>
      ...
      <div className="space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-primary-dark mb-2">
            Select {productType === 'rice' ? 'Rice Variety' : 'Spice/Powder'}
          </label>
          <select
        ...
        <div>
          <label className="block text-sm font-medium text-primary-dark mb-2">
            Source {productType === 'rice' ? 'Mill/Warehouse' : 'Processing Unit'}
          </label>
          <select
          ...
          <div className="mt-8 bg-primary-lighter rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between border-b border-primary-light pb-2">
                ...
                <span>{calculations.productName}</span>
              </div>

              {calculations.variety && (
                <div className="flex justify-between border-b border-primary-light pb-2">
                  <span className="font-medium">Variety:</span>
                  <span>{calculations.variety}</span>
                </div>
              )}

              <div className="flex justify-between border-b border-primary-light pb-2">
                <span className="font-medium">Base Price:</span>
                <span>${calculations.originalPrice.toFixed(2)} {calculations.originalUnit}</span>
              </div>

              <div className="flex justify-between border-b border-primary-light pb-2">
                <span className="font-medium">Base Price per MT:</span>
                <span>${calculations.basePrice.toFixed(2)}</span>
              </div>
              ...
              <div className="mt-4 pt-4 border-t-2 border-primary-medium">
                ...
                <div className="flex justify-between pb-2">
                  ...
                  <span className="text-sm">{calculations.sourceName} → {calculations.portName}</span>
                </div>
            ...
            <p className="text-xs text-primary-medium mt-4">
              * Prices are indicative and subject to market fluctuations. Final prices confirmed upon order.
              <br />
              * Last updated: {products.find(p => p.name === calculations.productName)?.lastUpdated}
            </p>
          </div>
        )}
      </div>
